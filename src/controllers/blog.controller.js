...
module.exports.BlogPost={
    ...
    create: async(req,res)=>{
        ...
        res.status(201).send({
            error:false,
            body:req.body,//kaydettiğim veri bu
            data:data// bu ise kaydettikten sonra tekrardan veriyi basıyor ekrana kaydettinde ne oldu diye

        })

    },
    read: async(req,res)=>{
        const data=await BlogPost.find({_id:req.params.postId})//id'si postid eşit olanı buldu
        res.status(202).send({
            error:false,
            data:data 
    ...
    update: async(req,res)=>{
        ...
        res.status(202).send({
            error:false,
            body:req.body,
            data:data, // bu kaç satırda update olduğunu yazar sadece veriyi dönmez 
            // güncel veriyi istiyorsan tekrar çağır
            newdata:newdata
